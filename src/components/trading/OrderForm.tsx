    ...
    <div className="flex flex-col h-full bg-white text-gray-900">
      ...
      {selectedScrip && (
        <div className="flex-1 overflow-y-auto px-3 py-3 flex flex-col gap-3">
          ...
          {/* ── Concurrent Trade Warning ── */}
          {hasOpenPositions && openPositionSymbol && selectedScrip.p_trd_symbol !== openPositionSymbol && (
            <div className="bg-amber-50 border border-amber-200 rounded-lg px-3 py-2 flex items-start gap-2">
              <AlertCircle size={14} className="text-amber-600 mt-0.5 flex-shrink-0" />
              <div className="text-xs text-amber-800">
                <p className="font-semibold mb-0.5">⚠️ Concurrent Trades Disabled</p>
                <p className="text-amber-700">
                  You have an open position in <span className="font-bold">{openPositionSymbol}</span>. 
                  Close it before opening a new position.
                </p>
              </div>
            </div>
          )}
